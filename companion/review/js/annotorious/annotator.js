...
function showModalPopup(title, text, submit, time, next) {
    ...
    if (submit) {
        $("#modal-request-button").removeClass("modal-request-hidden");
    }
    else {
        $("#modal-request-button").addClass("modal-request-hidden");
    }
    if (next) {
        $("#modal-request-next").removeClass("modal-request-hidden");
    }
    else {
        $("#modal-request-next").addClass("modal-request-hidden");
    }
    if (time) {
        setTimeout(function() {
            $("#myModal").modal("hide");
...
$("#annotator-submit").on("click", function() {
    xhr = checkLock();
    xhr.onload = function() {
        if (xhr.responseText == 'success' || xhr.responseText == 'access'){
            showModalPopup("Please wait. We are processing your request...");
                    ...
                    xhr2.onreadystatechange = function() {
                        if (xhr2.readyState === 4) {
                            if (xhr2.status === 200) {
                                ...
                                var submitResponse = xhr2.responseText;
            ...
            xhr1.send(JSON.stringify({saveData: {"pointers": pointers, "tags": json["tags"]}}));
        }
        else if (xhr.responseText == 'noaccess'){
            showModalPopup("You can't edit this document!", "This Document is locked because other user edit this document", false, 2000);
            return;
        }
    };
    xhr.onerror = function() {
...
function nextUUID() {
    var url = location.protocol + "//" + dash_server + ":" + dash_port + "/fetch/" + directory;
    var xhr = createCORSRequest("POST", url);
    if (!xhr) {
        throw new Error('CORS not supported');
    }
...
function cancelDoc() {
    if (directory == 'annotator')
        ...
        xhr.onload = function() {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    showModalPopup("Do you want to review the next document?", "This document released Lock from you. If you done work, please close this tab", false, undefined, true);
                } else {
                    showModalPopup("Your request could not be processed", xhr.responseText);
                }
            }
...
$(document).on("keydown", function(event) {
    ...
    if (event.keyCode === keyCodes.ENTER && $('#myModal').hasClass('in') && !$('#myModal .modal-body').hasClass('modal-request-hidden')) {
        if (!$("#modal-request-button").hasClass('modal-request-hidden'))
            $("#annotator-submit").click();
        else if (!$("#modal-request-next").hasClass('modal-request-hidden'))
            nextUUID();
        return;
    }
...
function changeSettingTypeOptions(optionList) {
    ...
    if (optionList) {
        ...
        settingType.removeAttribute('dataF-role');
...
function checkLock() {
    var url = location.protocol + "//" + dash_server + ":" + dash_port + "/getlock/" + directory;
    var xhr = createCORSRequest("POST", url);
    if (!xhr) {
        throw new Error('CORS not supported');
    }
...
function startup() {
    if (directory === "annotator") {
        showModalPopup("Do you want to review the next document?", undefined, false, undefined, true);
        return;
    }
    var xhttp;
    if (window.XMLHttpRequest) {
